...
const subTabCSS = `
    ...
    .citas-subtab { padding:12px 16px;font-family:var(--font-body);font-size:13px;font-weight:600;color:#9ca3af;background:none;border:none;border-bottom:2.5px solid transparent;margin-bottom:-2px;cursor:pointer;display:flex;align-items:center;gap:6px;transition:all 0.2s; }
    .citas-subtab.active { color:var(--brand);border-bottom-color:var(--brand); }
    .stats-bar-inline { margin-left:auto;display:flex;gap:12px;font-size:12px;font-weight:600; }
    .stat-inline { color:#6b7280; }
    .stat-inline.pending { color:#f59e0b; }
    .stat-inline.confirmed { color:#10b981; }
    .stat-inline.today { color:#6366f1; }
`;
c = c.replace('  </style>\n</head>', subTabCSS + '  </style>\n</head>');

// Replace admin-citas-view - just the inner structure
const oldStats = `      <div class="stats-bar">
...
const newStats = `      <!-- Sub-tabs Semana / Lista -->
      <div class="citas-subtabs">
        ...
        <div class="stats-bar-inline">
          <span class="stat-inline"><span id="stat-total">0</span> total</span>
          <span class="stat-inline pending"><span id="stat-pending">0</span> pend.</span>
          <span class="stat-inline confirmed"><span id="stat-confirmed">0</span> conf.</span>
          <span class="stat-inline today"><span id="stat-today">0</span> hoy</span>
        </div>
      </div>
      ...
      <div id="admin-cal-view">
        <div class="cal-toolbar">
          ...
          <select class="filter-select" id="cal-filter-location" onchange="renderAdminCalendar()" style="max-width:140px;margin-left:8px">
            <option value="">Todas</option>
            <option value="belmont">Belmont</option>
            <option value="ventura">Ventura</option>
            <option value="kings">Kings Canyon</option>
          </select>
        </div>
...
if (c.includes(oldStats)) {
    ...
    console.log('OLD STATS NOT FOUND');
}
